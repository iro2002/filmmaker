...
export default function Producer() {
  ...
  return (
    <section
      ...
      // FIX 1: Removed heavy padding from the main wrapper. Just keep it as the bounding box.
      className="relative w-full bg-[#050505] text-white overflow-hidden select-none"
    >
      <motion.div
        animate={{ opacity: activeCategory || activeProject ? 0 : 1 }}
        transition={{ duration: 0.5 }}
        // FIX 2: Changed from `absolute inset-0` to `relative`. Brought the padding and min-height here.
        // This puts the list into the document flow, preventing overlaps with adjacent sections.
        className="relative z-0 flex flex-col justify-center w-full min-h-[80vh] pt-24 pb-24 md:pb-48"
      >
        <div className="relative z-10 w-full flex flex-col gap-4 md:gap-10 mt-1 md:mt-0">
          <h2 className="text-right text-4xl sm:text-8xl md:text-7xl lg:text-[7rem] font-serif tracking-tighter mb-4 px-8 md:px-16 text-white/90 underline decoration-1 underline-offset-8 decoration-white/40">
            Producing
          </h2>
          {producerData.map((item, index) => (
            <motion.div
              ...
              className="group relative flex flex-row items-center justify-between border-b-0 md:border-b border-zinc-800/60 py-12 px-8 md:px-16 md:py-0 md:pb-8 cursor-pointer overflow-hidden md:overflow-visible"
            >
              ...
              <div className="relative z-10 flex flex-col items-end opacity-100 md:opacity-0 md:-translate-x-4 transition-all duration-500 ease-out md:group-hover:opacity-100 md:group-hover:translate-x-0">
                {/* Matched Gallery Small UI Font Style */}
      ...
      <AnimatePresence>
        {activeCategory && !activeProject && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            // FIX 3: Added `overflow-y-auto overflow-x-hidden` instead of just `overflow-hidden` 
            // to allow scrolling if a category has many items or on extremely short screens.
            className="fixed inset-0 z-40 bg-black flex flex-col h-screen overflow-y-auto overflow-x-hidden pt-24"
          >
            <div className="absolute top-0 left-0 w-full z-20 flex justify-between items-center p-6 md:p-12 pointer-events-none text-white mt-24 md:mt-12 bg-gradient-to-b from-black/80 to-transparent">
              {/* Matched Sub-heading Font Style */}
              <h2 className="text-3xl md:text-5xl lg:text-7xl font-serif tracking-tighter drop-shadow-lg">
                {activeCategory.title}
              </h2>
              <button
                onClick={() => setActiveCategory(null)}
                className="pointer-events-auto group flex items-center gap-4 text-[10px] md:text-xs font-bold tracking-widest uppercase hover:text-white/70 transition-colors"
              >
            ...
            <div className={`group flex flex-col md:flex-row w-full min-h-full flex-grow`}>
              {activeCategory.projects.map((proj) => (
                <motion.div
                  ...
                  className="relative flex-1 min-h-[50vh] cursor-pointer overflow-hidden border-b border-zinc-900 md:border-b-0 md:border-r last:border-0 hover:!opacity-100 group-hover:opacity-30 transition-all duration-500 ease-out"
                >
                  <img
                    src={proj.thumbnail}
                    alt={proj.title}
